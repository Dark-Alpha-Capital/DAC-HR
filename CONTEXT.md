...
- **Round progression** (`packages/db/src/round-progression.ts`) — pure bundle round-decision rules (which round is active, which is next, when a bundle is done, delivery-mode coercion). Repositories and routes adapt DB rows to it.
