...
export const applicationStatusDescriptions = {
...
export const applicationStatusBadgeVariants = {
...
} satisfies Record<ApplicationStatus, "default" | "secondary" | "destructive">;

export const applicationStatusKanbanColors = {
...
export const applicationStatusBorderColors = {
...
export const applicationStatusChartColors = {
  ai_screening: "#6366F1",
  first_round: "#3B82F6",
...
const legacyApplicationStatusLabels = {
...
export const legacyApplicationStatusMap = {
  ...
  withdrawn: "rejected",
} satisfies Record<string, ApplicationStatus>;
...
export function isApplicationStatus(value: string): value is ApplicationStatus {
  // SAFETY: `applicationStatuses` is a string-literal array; widening to
...
export function normalizeApplicationStatus(
  ...
  return (
    legacyApplicationStatusMap[
      // SAFETY: `status` is an arbitrary string; the key cast narrows it to
      // the known legacy keys, and unknown keys index to `undefined` → null.
...
export function getApplicationStatusLabel(status: string): string {
  ...
  if (status in legacyApplicationStatusLabels) {
    // SAFETY: the `in` check narrows `status` to a known legacy label key.
    return legacyApplicationStatusLabels[
      status as keyof typeof legacyApplicationStatusLabels
    ]!;
  }
