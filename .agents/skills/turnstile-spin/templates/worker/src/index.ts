...
async function parseBody(request: Request): Promise<SiteverifyRequest> {
  ...
  if (contentType.includes("application/json")) {
    ...
    return {
      ...
      token: String(
        body.token ?? body["cf-turnstile-response"] ?? body.response ?? "",
      ),
  ...
  if (
    ...
    return {
      ...
      idempotency_key: form.get("idempotency_key")
        ? String(form.get("idempotency_key"))
        : undefined,
    };
  }
...
async function handleSiteverify(request: Request, env: Env): Promise<Response> {
  ...
  const remoteip =
    parsed.remoteip ?? request.headers.get("CF-Connecting-IP") ?? undefined;
  const requestForValidate: SiteverifyRequest = { ...parsed, remoteip };

  try {
    const { response, upstreamStatus, durationMs } = await validate(
      requestForValidate,
      env,
      WORKER_VERSION,
    );
    ...
    if (request.method === "GET" && url.pathname === "/post-deploy") {
      if (env.ASSETS) {
        return env.ASSETS.fetch(
          new Request(new URL("/post-deploy.html", url.origin)),
        );
      }
    }

    const isSiteverifyPath =
      url.pathname === "/" || url.pathname === "/siteverify";
