...
const candidate = new Hono()
   ...
   */
  .delete("/bulk", authMiddleware, async (c) => {
    const startTime = Date.now();
    try {
      const user = c.get("user");
      if (!user) {
        console.error(
          "[DELETE /candidate/bulk] Unauthorized request - no user found"
        );
        return c.json({ error: "Unauthorized" }, { status: 401 });
      }
      ...
      try {
        ...
        console.error("[DELETE /candidate/bulk] Invalid request body:", error);
        return c.json(
          {
            error:
      ...
      if (
        ...
        console.error(
          "[DELETE /candidate/bulk] Missing or invalid candidateIds"
        );
        return c.json(
      ...
      console.log(
        `[DELETE /candidate/bulk] Bulk deleting ${candidateIds.length} candidates - User: ${user.email} (${user.id})`
      );
      ...
      for (const candidateId of candidateIds) {
        try {
          ...
          if (!candidateData) {
            console.error(
              `[DELETE /candidate/bulk] Candidate not found - ID: ${candidateId}`
            );
            results.push({
              id: candidateId,
              success: false,
          ...
          const candidateDocuments =
            await getDocumentsByCandidateId(candidateId);
            ...
            if (doc.url) {
              promises.push(
                deleteFile(doc.url).catch((error) => {
                  console.error(
                    `[DELETE /candidate/bulk] Error deleting file from GCS for document ${doc.id}:`,
                    error
                  );
                  return false;
                })
            ...
            if (doc.fileSearchDocumentName) {
              promises.push(
                deleteFileSearchStoreDocument(doc.fileSearchDocumentName).catch(
                  (error) => {
                    console.error(
                      `[DELETE /candidate/bulk] Error deleting file from FileSearchStore for document ${doc.id}:`,
                      error
                    );
                    return false;
                  }
          ...
          console.error(
            `[DELETE /candidate/bulk] Error deleting candidate ${candidateId}:`,
            error
          );
          results.push({
            id: candidateId,
      ...
      const duration = Date.now() - startTime;
      console.log(
        `[DELETE /candidate/bulk] Request completed in ${duration}ms - Success: ${successCount}, Failed: ${failedCount}`
      );
      ...
      const duration = Date.now() - startTime;
      console.error(
        `[DELETE /candidate/bulk] Error processing bulk delete after ${duration}ms:`,
        error
      );
      return c.json(
        {
